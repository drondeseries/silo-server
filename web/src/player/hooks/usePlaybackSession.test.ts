...
describe("usePlaybackSession replans", () => {
  ...
  it("audio track_change keeps the same transport when both plans carry renditions", async () => {
    const renditions = [
    ...
    const audioSwitchPlan = fixturePlanV3({
      plan_id: "plan:2222222222222222",
      plan_attempt_key: "v3:2222222222222222",
      audio_renditions: renditions,
    ...
    act(() => result.current.switchAudioTrack(2, 120));
    await waitFor(() => expect(result.current.plan?.plan_id).toBe("plan:2222222222222222"));
    expect(replanBodies[0]?.operation).toBe("track_change");
    expect(result.current.planRevision).toBe(2);
    // Both plans carry the same rendition set on the same stream URL: the
    // audio selection is a rendition swap, not a transport change — the player
    // must keep the mounted element.
    expect(result.current.transportRevision).toBe(1);

    unmount();
  });

  it("audio track_change bumps the transport when renditions are absent", async () => {
    const initialPlan = fixturePlanV3();
    const audioSwitchPlan = fixturePlanV3({
      plan_id: "plan:2222222222222222",
      plan_attempt_key: "v3:2222222222222222",
    ...
    act(() => result.current.switchAudioTrack(1, 120));
    await waitFor(() => expect(result.current.plan?.plan_id).toBe("plan:2222222222222222"));
    // No renditions: an audio index change is a real A/V transport difference
    // and must bump the transport (conservative, no stale-audio risk).
