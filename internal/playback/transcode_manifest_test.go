...
func TestBuildAudioRenditionsMasterManifest(t *testing.T) {
	...
	renditions := []AudioRenditionV3{
		{Index: 0, TrackID: "file:42:audio:0", Language: "en", Languages: []string{"en", "fr", "de"}, Codec: "eac3", Default: true},
		{Index: 1, TrackID: "file:42:audio:1", Language: "ja", Codec: "aac"},
	}

	master := string(session.BuildAudioRenditionsMasterManifest(renditions, "token=test"))
	for _, want := range []string{
		"#EXTM3U",
		`#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID="aud",NAME="en",DEFAULT=YES,AUTOSELECT=YES,LANGUAGE="en",URI="audio_0/audio.m3u8"?token=test`,
		`#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID="aud",NAME="ja",DEFAULT=NO,AUTOSELECT=NO,LANGUAGE="ja",URI="audio_1/audio.m3u8"?token=test`,
		`#EXT-X-STREAM-INF:BANDWIDTH=12000000,AUDIO="aud"`,
		"video/video.m3u8?token=test",
	} {
...
func TestBuildAudioRenditionsMasterManifestFallsBackToDefaultBandwidth(t *testing.T) {
	session := &TranscodeSession{opts: TranscodeOpts{TargetCodecVideo: "copy"}}
	master := string(session.BuildAudioRenditionsMasterManifest([]AudioRenditionV3{
		{Index: 0, Language: "en", Default: true},
	}, ""))
	if !strings.Contains(master, "BANDWIDTH=5000000") {
		t.Fatalf("master manifest missing default bandwidth:\n%s", master)
	}
	if strings.Contains(master, "?") {
		t.Fatalf("master manifest should carry no query when rawQuery is empty:\n%s", master)
	}
}

func TestGenerateAudioRenditionManifestMirrorsFullVODPlaylist(t *testing.T) {
	opts := TranscodeOpts{TargetCodecVideo: "h264", SegmentDuration: 2, TotalDuration: 5.1}
	session := &TranscodeSession{opts: opts}

	rendition := string(session.GenerateAudioRenditionManifest("segment/", "token=test"))
	for _, want := range []string{
		"#EXT-X-PLAYLIST-TYPE:VOD",
		"#EXT-X-MEDIA-SEQUENCE:0",
		"segment/seg_00000.ts?token=test",
		...
		if !strings.Contains(rendition, want) {
			t.Fatalf("rendition playlist missing %q:\n%s", want, rendition)
		}
	}
	// The rendition playlist and the primary synthetic manifest are produced by
	// the same VOD producer, so the existing single-rendition manifest output is
	// byte-identical to what this delivers per rendition.
	primary := string(session.GenerateFullManifest("segment/", "token=test"))
	if rendition != primary {
		t.Fatalf("rendition playlist diverged from the primary synthetic manifest:\n%s", rendition)
	}
}

func TestAudioRenditionsRecipeBuilderGatedOff(t *testing.T) {
	if AudioRenditionsEnabled {
		t.Skip("renditions delivery is enabled; gating test is moot")
	}
	if args := buildAudioRenditionsFFmpegArgs(TranscodeOpts{
		TargetCodecVideo: "copy",
		AudioRenditions:  []AudioRenditionV3{{Index: 0}, {Index: 1}},
	}); args != nil {
		t.Fatalf("recipe builder produced arguments while AudioRenditionsEnabled is false: %v", args)
	}
}
