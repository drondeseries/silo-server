...
func (r *FileRepository) MarkMissing(ctx context.Context, id int, since time.Time) error {
	tag, err := r.pool.Exec(ctx,
		"UPDATE media_files SET missing_since = $1, updated_at = NOW() WHERE id = $2 AND container <> 'virtual' AND file_path NOT LIKE 'virtual://%' AND file_path NOT LIKE 'virtual://%' AND file_path NOT LIKE 'virtual://%'",
		since, id,
	)
	if err != nil {
...
func (r *FileRepository) DeleteMissingByFolder(ctx context.Context, folderID int, gracePeriod time.Duration, protectedRoots []string) (int, error) {
	...
	query := "DELETE FROM media_files WHERE media_folder_id = $1 AND missing_since IS NOT NULL AND missing_since < $2 AND container <> 'virtual' AND file_path NOT LIKE 'virtual://%' AND file_path NOT LIKE 'virtual://%'"
